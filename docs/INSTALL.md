...
The installer copies these Kona-owned trees: Hyprland, Kitty, Kona, Quickshell, Rofi, SwayNC, SwayOSD, systemd user configuration, Waybar, XDG portal configuration, Kona executables, desktop entries and wallpaper assets. Every existing target is copied to `~/.local/state/kona/pre-restore-<timestamp>/` before replacement.
...
The helper downloader uses Arch package URLs to unpack SwayOSD, Satty, wf-recorder, Hyprsunset and Quickshell below `~/.local/opt/kona-pkgs/`. System package installation remains the user's responsibility.
