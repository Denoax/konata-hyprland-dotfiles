...
`kona-theme` validates contrast and atomically renders fragments for Waybar, Rofi, SwayOSD, Foot, Kitty, Hyprland and Quickshell. `.config/kona/theme/current/` is generated and should not be hand-edited.
...
Kona Settings uses Caelestia Nexus for the local wallpaper browser, but every selection is applied and persisted by `kona-profile`; Nexus does not own wallpaper or theme state. The adjacent Wallpaper Engine entry retains Kona's scene and Steam Workshop picker.
...
Copy `.config/kona/weather.example.json` to `~/.config/kona/weather.json` and set an explicit display name, region, latitude and longitude. `units` accepts `metric` or `imperial`; `refresh_minutes` accepts 15–120. The sidebar reads the cached summary once when it starts. Opening Weather passes the same coordinates and units to the transient Caelestia dashboard, avoiding its IP-location fallback. No location service, API key or resident weather process is used.
...
The Arch system terminal uses `.config/fastfetch/kona-arch.jsonc` and the tracked Kitty image asset under `.config/fastfetch/assets/`. It keeps Kitty for native image support. Normal and scratch terminals use Foot, Fish and Starship with `.config/foot/foot.ini` and `.config/starship.toml`.
...
The sidebar source is `.config/quickshell/kona/sidebar/`. Shared primitives and semantic token readers live under its `qml/components/` tree. Menu geometry belongs to each Rofi/Quickshell/SwayOSD surface; color roles come from the generated semantic state. Do not create a second palette or appearance state for one menu.
