...
QtObject {
    ...
    property var palette: ({mode:"light", background:"#EEF4FC", surface:"#F7FAFF", surface_alt:"#EEF4FC",
        surface_elevated:"#FFFFFF", surface_pressed:"#E3EDF9", text:"#263446", text_secondary:"#5B6B80",
        text_muted:"#8A98AA", accent:"#78A9FF", accent_hover:"#8CB7FF", accent_pressed:"#6498F5",
        accent_soft:"#DDEAFF", outline:"#C9DBF3", outline_strong:"#AFC9EB", focus:"#6FA6FF",
        danger:"#D95E6A", warning:"#C88932", success:"#28A875", shadow:"rgba(36,59,89,0.16)",
    ...
    function accept(value) {
        const required=["mode","surface","surface_alt","surface_elevated","surface_pressed","text","text_secondary",
            "text_muted","accent","accent_soft","outline","outline_strong","focus","shadow","glow","scrim"];
        if(!value || !["light","dark"].includes(value.mode) || required.some(key => typeof value[key] !== "string"))
            throw new Error("invalid semantic appearance palette");
        root.palette=value; root.mode=value.mode;
    }
    property FileView source: FileView {
        path: root.configRoot + "/kona/appearance/current.json"; watchChanges: true; printErrors: false
        onFileChanged: reload()
        onLoaded: { try { root.accept(JSON.parse(text())); } catch(error) { console.warn("Weather appearance:",error); } }
    }
}
